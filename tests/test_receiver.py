...
async def simple_test(dut):
    ...
    ideal_SSS_sym *= np.exp(1j * 2 * np.pi * (CP2_LEN - CP_ADVANCE) / FFT_SIZE * np.arange(FFT_SIZE))
    ...
    received_PBCH_ideal *= np.exp(1j * 2 * np.pi * (CP2_LEN - CP_ADVANCE) / FFT_SIZE * np.arange(FFT_SIZE))
