...
# Overview
This is a verilog HDL core for a 5G NR lower Phy receiver. It is optimized for low ressource usage so that it can run on a [PlutoSDR](https://www.analog.com/en/design-center/evaluation-hardware-and-software/evaluation-boards-kits/adalm-pluto.html), which has a Xilinx® Zynq Z-7010 with only 80 DSP slices and 28K logic cells. Since this design is optimized for low ressource usage, it can demodulate at 3.84 MSPS (256-FFT), in that case 20 RBs (Ressource Blocks) are usable. Another option is to run this design at 7.68 MHz (512-FFT), in that case 40 RBs are usable which is enough to accomodate the smallest possible CORESET which uses 24 RBs. A ressource block in 5G consists of 12 subcarriers. A 256 point FFT contains 20*12=240 usable subcarriers, because at each edge of the spectrum there are 8 zero carriers. A 512 point FFT contains 40*12=480 usable subcarriers.<br>
